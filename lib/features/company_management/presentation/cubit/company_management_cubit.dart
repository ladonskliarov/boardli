...
class CompanyManagementCubit extends Cubit<CompanyManagementState> {
  ...
  CompanyManagementCubit({required this.companyManagementRepository}) : super(CompanyManagementInitial());
  ...
  Future<void> loadOrganizationData() async {
    ...
    final departmentsResult = await companyManagementRepository.getDepartments();
    ...
    employeesResult.fold(
      ...
      (employees) {
        departmentsResult.fold(
          (failure) => emit(CompanyManagementFailure(message: failure.message)),
          (departments) => emit(CompanyManagementLoaded(employees: employees, departments: departments)),
        );
      },
    );
  }

  Future<void> createEmployeeInvite({required String name, required String email, required String department, required String role}) async {
      ...
      result.fold(
        (failure) => emit(CompanyManagementInitial()),
        (message) => loadOrganizationData(),
      );
  }

  Future<void> createDepartment({required String department}) async {
      final result = await companyManagementRepository.createDepartment(department: department);
